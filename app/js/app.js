     ...
     return `
       <div class="proposal-site">
         <nav class="proposal-nav" id="proposalNav">
           <div class="nav-inner">
             <div class="nav-brand"><strong>\u63d0\u6848 OS</strong><span>Editorial Presentation System</span></div>
...
window.MediaSlots = {
   enhanceAll(selector) {
     document.querySelectorAll(selector).forEach(target => {
       if (target.dataset.enhanced === 'true') return;
       target.dataset.enhanced = 'true';
       this._appendZone(target);
       const slotName = target.dataset.slot;
       const saved = window.AppState.media[slotName];
       if (saved) this.setImage(slotName, saved);
     });
   },

   fillMany(slotNames, files) {
     Array.from(files || []).slice(0, slotNames.length).forEach((file, index) => {
       this._readFile(file, src => this.setImage(slotNames[index], src));
     });
   },

   setImage(slotName, src) {
     const target = document.querySelector('[data-slot="' + slotName + '"]');
     if (!target) return;
     window.AppState.media[slotName] = src;

     let img = target.querySelector('.slot-image');
     if (!img) {
   ...
   _appendZone(target) {
     const zone = document.createElement('div');
     zone.className = 'img-upload-zone';
     zone.innerHTML = '<input type="file" accept="image/*"><div class="upload-icon">\u2295</div><div class="upload-text">\u66ff\u6362\u56fe\u7247</div><div class="upload-hint">' + target.dataset.slot + '</div>';
     const input = zone.querySelector('input[type="file"]');
     input.addEventListener('change', () => {
       const file = input.files[0];
       if (!file) return;
       this._readFile(file, src => this.setImage(target.dataset.slot, src));
     });
